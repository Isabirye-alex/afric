...
class _BarCodeScreenState extends State<BarCodeScreen> with WidgetsBindingObserver {
  ...
  String barcodeText = 'Scan Barcode...';
  ...
  Future<void> _processCameraImage(CameraImage image) async {
    if (_frameCount++ % _frameSkip != 0) {
      return;
        ...
        debugPrint(
            'Processing image: format=${image.format.group}, raw=${image.format
                .raw}, '
                'width=${image.width}, height=${image.height}, bytesPerRow=${image
                .planes.first.bytesPerRow}');
        ...
        try {
          ...
          final deviceOrientation = await NativeDeviceOrientationCommunicator()
              .orientation();
          InputImageRotation rotation = _calculateInputRotation(
  ...
  Future<void> _saveFrameForDebugging(Uint8List bytes, int width,
      int height) async {
    try {
      final directory = await getTemporaryDirectory();
      final filePath = '${directory.path}/debug_frame_${DateTime
          .now()
          .millisecondsSinceEpoch}.yuv';
  ...
  InputImageRotation _calculateInputRotation(int sensorOrientation,
                ...
                body: Stack(
                  children: [
                    GestureDetector(
                      onTapDown: (details) {
                        final offset = Offset(
                          details.localPosition.dx / MediaQuery
                              .of(context)
                              .size
                              .width,
                          details.localPosition.dy / MediaQuery
                              .of(context)
                              .size
                              .height,
                        );
